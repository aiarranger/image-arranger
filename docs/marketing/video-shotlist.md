...
| 7 | 0:33–0:38 | Video tab: create `aichan-locked-camera-8s` only after an adopted scene still exists; for a fixed-camera clip, choose the same adopted scene still as both start and end, duration 8s, then queue | entry form `formFramePicker`, `requestTarget` video branch |
