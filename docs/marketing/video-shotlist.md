...
> sample deck v2 (Aoi, all tabs alive, pre-seeded queue), and `npm run demo-agent`
> for deterministic 3-second turnarounds. Every shot below is now filmable as written.
...
| 4 | 0:14–0:20 | Time-lapse beat: agent works in terminal; **app updates by itself** — badge flips to done, arrival toast, thumbnail lands with a "new" ring | agent → `POST /api/requests/complete`; livePoll picks it up within 5s. For deterministic takes use `npm run demo-agent` |
...
| 6 | 0:27–0:33 | Improve loop: click "この画像を改善", type "笑顔を強く", pick 作り直し, queue → fly-to-queue again. Payoff: A/B compare slider on the improved result | `[data-edit-improve-asset]` → `openAsset`, improveMode radios, `#queueImproveAsset` → `improveTarget`; Compare control in the entry modal |
