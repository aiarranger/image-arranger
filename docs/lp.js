...
(() => {
  ...
  const I18N = {
    en: {
      ...
      wf_video_body: "Queue video only after a real adopted scene still exists. For locked-camera motion, use a start frame by itself or reuse the same adopted still as both start and end.",
      wf_video_step_one: "Requires a real adopted scene still.",
      wf_video_step_two: "Uses an explicit startFrame path and an optional endFrame path.",
      wf_video_step_three: "Shows no completed clip until a real MP4 is registered.",
      wf_video_caption: "Video view: no fake completed clips; frame fields point to adopted stills from the same scene.",
    ...
    ja: {
      ...
      wf_video_body: "動画キューは、実在する採用済みシーン画像ができてから使います。固定カメラなら startFrame だけ、または同じ採用済み画像を start/end に指定します。",
      wf_video_step_one: "実在する採用済みシーン画像が必要です。",
      wf_video_step_two: "startFrame に明示的なファイルパスを入れ、必要な場合だけ endFrame も入れます。",
      wf_video_step_three: "実在するMP4が登録されるまで、完成動画として見せません。",
      wf_video_caption: "Video 画面：完成済み動画を装わず、フレーム指定は同一シーンの採用済み静止画だけを参照します。",
      wf_queue_label: "05 · QUEUE",
